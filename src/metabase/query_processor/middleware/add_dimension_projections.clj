...
(mu/defn- col->dim-map :- [:maybe InternalDimensionInfo]
  ...
  (when (seq values)
    (let [remap-from       (:name col)
          stringified-idxs (qp.store/miscellaneous-value [::large-int-id/field-indexes])]
      {:col-index       idx
       :from            remap-from
       :to              remap-to
       :value->readable (zipmap (transform-values-for-col col values (and stringified-idxs (stringified-idxs idx)))
