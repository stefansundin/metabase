        ...
        :from [[{:union-all (if api/*is-superuser?*
                              ...
                              [{:select [:c.*]
                               ...
                               {:select [[:c.*]]
                                :from [[:collection :c]]
                                :where [:= :type "trash"]}
                               (let [personal-collection-and-descendant-ids
                                     (some-> api/*current-user-id* user->personal-collection-and-descendant-ids)]
                                 {:select [:c.*]
                                  :from [[:collection :c]]
                                  :where (if (seq personal-collection-and-descendant-ids)
                                           [:in :id personal-collection-and-descendant-ids]
                                           [:= 0 1])})])}
                :c]]
